...
export default function RomanticWebsite() {
  ...
  const [particles, setParticles] = useState<{ id: number; left: string; delay: string; size: string; duration: string; content: string }[]>([]);
  const [floatingHearts, setFloatingHearts] = useState<{ id: number; left: string; delay: string; duration: string; size: string; content: string }[]>([]);
  
  // Audio
  const [isPlaying, setIsPlaying] = useState(false);
  const audioRef = useRef<HTMLAudioElement | null>(null);

  // 1. Generadores de partículas estables
  useEffect(() => {
    // Corazones morados/rosa flotantes (Para la Portada estilo TikTok)
    setFloatingHearts(Array.from({ length: 25 }).map((_, i) => ({
      id: i, 
      left: `${Math.random() * 100}%`, 
      delay: `${Math.random() * 6}s`, 
      duration: `${Math.random() * 6 + 6}s`, 
      size: `${Math.random() * 1.2 + 0.6}rem`,
      content: Math.random() > 0.5 ? "💜" : "💟" // Definido aquí para evitar errores visuales
    })));

    // Partículas cinemáticas MIXTAS (Para el Diario Interior) - Gatos y Enfermería
    const particleTypes = ["✨", "🐾", "➕"]; 
    setParticles(Array.from({ length: 35 }).map((_, i) => ({
      id: i, 
      left: `${Math.random() * 100}%`, 
      delay: `${Math.random() * 6}s`, 
      size: `${Math.random() * 16 + 8}px`, 
      duration: `${Math.random() * 8 + 6}s`,
      content: particleTypes[Math.floor(Math.random() * particleTypes.length)]
    })));
  }, []);
  ...
  // ==============================================================================
  // --- PANTALLA 1: BLOQUEO / ESPERA (CORAZONES CORREGIDOS) ---
  // ==============================================================================
  if (!isUnlocked) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#e91e63] to-[#880e4f] text-white font-sans flex flex-col items-center justify-center py-10 px-4 relative overflow-hidden selection:bg-purple-500 selection:text-white">
        ...
        {/* Fondo de Corazones Flotantes Corregido */}
        <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
          ...
          {floatingHearts.map((h) => (
            <span key={h.id} className="absolute bottom-[-5%] animate-float-heart pointer-events-none" style={{ left: h.left, animationDelay: h.delay, animationDuration: h.duration, fontSize: h.size }}>
              {h.content}
            </span>
          ))}
        </div>
...
<span className="text-gray-500">7</span> {'}'}
          ...
          {modoEdicion && <p className="text-center text-xs text-red-100 font-bold bg-red-900/30 py-1.5 rounded-full border border-red-500/50">🔧 MODO ADMIN ACTIVADO</p>}
        </div>
        
        <style dangerouslySetInnerHTML={{__html: `
          @keyframes float-heart { 0% { transform: translateY(10vh) scale(0.5) rotate(0deg); opacity: 0; } 15% { opacity: 0.35; } 85% { opacity: 0.35; } 100% { transform: translateY(-120vh) scale(1.4) rotate(45deg); opacity: 0; } }
  ...
  const slidesContent = [
      ...
      <div className="bg-[#fdfbf7] text-[#333] rounded-[2rem] p-6 md:p-8 shadow-2xl relative overflow-hidden w-full border border-[#c084fc]/30">
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1 pr-2">
            ...
            <h1 className="text-3xl md:text-5xl font-serif font-bold text-[#333] mb-3 animate-nursing-pulse">Bienvenida, <br/><span className="text-[#e91e63]">mi Amor</span></h1>
    ...
    <div key="s1_video" className="flex flex-col items-center animate-slide-in w-full max-w-4xl">
      <h2 className="text-4xl font-serif text-[#c084fc] mb-4 animate-nursing-pulse">Nuestra magia en <span className="text-[#deff9a]">movimiento</span></h2>
    ...
    <div key="s2" className="flex flex-col md:flex-row items-center gap-10 animate-slide-in w-full max-w-5xl">
      <div className="flex-1">
        ...
        <p className="text-[#daffde] text-lg">Fue el día en que las palabras empezaron a tener otro peso y las miradas otro brillo <span className="animate-cat-waddle inline-block">🐾</span>.</p>
      </div>
      <div className="flex-1">
    ...
    <div key="s3" className="flex flex-col md:flex-row-reverse items-center gap-10 animate-slide-in w-full max-w-5xl">
      <div className="flex-1">
        ...
        <blockquote className="border-l-4 border-[#deff9a] pl-4 italic text-[#daffde]">"Hay viajes que no se miden en kilómetros, sino en momentos." <span className="animate-pulse inline-block text-red-400">❤️</span></blockquote>
      </div>
      <div className="flex-1">
    ...
    <div key="s5" className="flex flex-col md:flex-row items-center gap-12 animate-slide-in w-full max-w-5xl">
      <div className="flex-1 text-center">
        ...
        <p className="text-[#c084fc] text-sm">Y {timeTogether.hours} horas con {timeTogether.minutes} minutos <span className="animate-nursing-pulse inline-block text-white">✚</span>.</p>
      </div>
      <div className="flex-1 bg-[#1a0d35] p-8 rounded-2xl border border-[#c084fc]/30">
        ...
        <div className="space-y-6">
          {[ { label: 'Amor ❤️', w: '100%' }, { label: 'Risas 😂', w: '95%' }, { label: 'Planes 🚀', w: '100%' } ].map((bar, i) => (
            <div key={i} className="flex items-center gap-4">
              <span className="w-20 text-right text-[#daffde] font-bold text-xs">{bar.label}</span>
    ...
    <div key="s6" className="flex flex-col items-center animate-slide-in w-full max-w-5xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
        {[
          ...
          { icon: '🐾', title: 'Tu Brillo', text: 'Como un gatito curioso, siempre encuentras la alegría en las cosas pequeñas y llenas mi vida de ternura.' },
          { icon: '🚀', title: 'Tu Fuerza', text: 'Estudias para cuidar vidas, y esa misma determinación te hace la mujer más admirable que conozco.' }
        ].map((item, i) => (
          <div key={i} className="bg-[#1a0d35] p-8 rounded-2xl text-center border border-white/5 shadow-xl transition-all hover:border-[#c084fc]/50 hover:-translate-y-2">
    ...
    <div key="s7" className="flex flex-col items-center justify-center text-center animate-slide-in w-full max-w-3xl h-full">
      <div className="text-7xl text-[#c084fc] opacity-50 mb-4 animate-sunset-glow">"</div>
      ...
      <cite className="text-2xl text-[#deff9a] block animate-nursing-pulse">— Por siempre tuyo</cite>
    </div>,

    // Slide 9: Capítulo 2
    <div key="s8" className="flex flex-col items-center text-center animate-slide-in w-full max-w-4xl">
      {(canSeeChapter2 || modoEdicion) ? (
        <div className="bg-[#1a0d35] border border-[#deff9a]/50 p-10 rounded-[2rem] shadow-[0_0_30px_rgba(222,255,154,0.2)] relative overflow-hidden">
          {modoEdicion && !canSeeChapter2 && <span className="absolute top-2 right-4 text-xs text-red-500 font-bold">Vista Previa Admin</span>}
          <div className="absolute inset-0 bg-gradient-to-br from-purple-900/50 to-orange-900/30 animate-sunset-cycle z-0" />
          <div className="relative z-10">
            <h2 className="text-4xl font-serif text-[#deff9a] mb-4 animate-nursing-pulse">Capítulo 2: Desbloqueado <span className="animate-pulse">🌟</span></h2>
            ...
            <p className="text-[#f5f5f5] text-lg">Gracias por seguir llenando estos días de alegría <span className="animate-cat-waddle inline-block">🐾</span>. Este diario seguirá creciendo con nosotros cada mes.</p>
          </div>
        </div>
      ) : (
        <div className="bg-[#1a0d35]/50 border-2 border-dashed border-[#c084fc]/40 p-12 rounded-[3rem] relative overflow-hidden">
          ...
          <p className="text-[#c084fc] text-lg font-bold">Vuelve a escanear tu llave el <span className="animate-sunset-glow">18 de Junio</span> a las 12:00 PM.</p>
        </div>
      )}
    </div>
  ];
  ...
  return (
    <div className="min-h-screen text-[#f5f5f5] font-sans selection:bg-[#c084fc] flex flex-col relative overflow-hidden bg-[#0f0720]">
      
      {/* --- FONDO CINEMÁTICO CON ATARDECER DINÁMICO --- */}
      <div className="absolute inset-0 z-0 pointer-events-none animate-cinematic-bg overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-b from-[#0f0720] via-[#1a0d35] to-[#2d1b4a] animate-sunset-cycle opacity-80" />
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_30%,_rgba(147,51,234,0.25)_0%,_transparent_60%),_radial-gradient(circle_at_80%_80%,_rgba(255,165,0,0.1)_0%,_transparent_50%)]" />
      </div>
      
      {/* --- PARTÍCULAS MIXTAS DEL DIARIO --- */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none z-0">
        {particles.map((p) => (
          <span key={p.id} className="absolute bottom-[-10%] text-[#c084fc]/50 animate-float-up inline-block pointer-events-none" style={{ left: p.left, animationDelay: p.delay, fontSize: p.size, animationDuration: p.duration }}>
            {p.content}
          </span>
        ))}
      </div>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        @keyframes cinematic-bg { 0%, 100% { transform: scale(1) translate(0px, 0px); } 50% { transform: scale(1.05) translate(10px, -10px); } }
        @keyframes float-up { 0% { transform: translateY(10vh) scale(0) rotate(0deg); opacity: 0; } 10% { opacity: 0.7; } 90% { opacity: 0.7; } 100% { transform: translateY(-120vh) scale(1) rotate(360deg); opacity: 0; } }
        
        @keyframes sunset-cycle { 
          0%, 100% { background-color: #0f0720; }
          25% { background-color: #2d1b4a; }
          50% { background-color: #5a2e61; }
          75% { background-color: #1a0d35; }
        }
        @keyframes sunset-glow {
          0%, 100% { text-shadow: 0 0 10px rgba(255,165,0,0.3); color: #c084fc; }
          50% { text-shadow: 0 0 20px rgba(255,100,0,0.6); color: #ffca80; }
        }
        @keyframes nursing-pulse {
          0%, 100% { transform: scale(1); text-shadow: 0 0 10px rgba(192,132,252,0.5); }
          10% { transform: scale(1.02); text-shadow: 0 0 15px rgba(192,132,252,0.7); }
          20% { transform: scale(1); text-shadow: 0 0 10px rgba(192,132,252,0.5); }
          30% { transform: scale(1.01); text-shadow: 0 0 12px rgba(192,132,252,0.6); }
          40% { transform: scale(1); text-shadow: 0 0 10px rgba(192,132,252,0.5); }
        }
        @keyframes cat-waddle {
        ...
        .animate-cinematic-bg { animation: cinematic-bg 25s ease-in-out infinite; }
        .animate-float-up { animation: float-up linear infinite; }
        .animate-sunset-cycle { animation: sunset-cycle 15s ease-in-out infinite; }
        .animate-sunset-glow { animation: sunset-glow 4s ease-in-out infinite; }
        .animate-nursing-pulse { animation: nursing-pulse 2s ease-in-out infinite; }
        .animate-cat-waddle { animation: cat-waddle 1s ease-in-out infinite; }
      `}} />
    </div>
  );
}